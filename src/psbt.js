...
class Psbt {
    ...
    addIssuance(args, inputIndex) {
        ...
        const issuance = issuance_1.newIssuance(args.assetAmount, args.tokenAmount, args.precision, args.contract);
        // generate the entropy
        const entropy = issuance_1.generateEntropy({ txHash: hash, vout: index }, issuance.assetEntropy);
        ...
        if (args.tokenAmount !== 0) {
            ...
            const token = issuance_1.calculateReissuanceToken(entropy, args.confidential);
    ...
    finalizeInput(inputIndex) {
        ...
        const { finalScriptSig, finalScriptWitness } = getFinalScripts(script, scriptType, input.partialSig, isSegwit, isP2SH, isP2WSH);
        if (finalScriptSig)
        ...
        for (const input of this.__CACHE.__TX.ins) {
            if (input.issuance) {
                const isConfidentialIssuance = issuanceBlindingPrivKeys && issuanceBlindingPrivKeys[inputIndex]
                    ? true
                    : false;
                const entropy = issuance_1.generateEntropy({ txHash: input.hash, vout: input.index }, input.issuance.assetEntropy);
                ...
                if (issuance_1.hasTokenAmount(input.issuance)) {
                    const token = issuance_1.calculateReissuanceToken(entropy, isConfidentialIssuance);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const input of this.__CACHE.__TX.ins) {
                if (input.issuance) {
                    ...
                    const assetCommitment = yield confidential.assetCommitment(blindingFactorsAsset.asset, blindingFactorsAsset.assetBlindingFactor);
                    const valueCommitment = yield confidential.valueCommitment(blindingFactorsAsset.value, assetCommitment, blindingFactorsAsset.valueBlindingFactor);
                    ...
                    const issuanceRangeProof = yield confidential.rangeProof(blindingFactorsAsset.value, assetBlindingPrivateKey, blindingFactorsAsset.asset, blindingFactorsAsset.assetBlindingFactor, blindingFactorsAsset.valueBlindingFactor, valueCommitment, Buffer.alloc(0), '1', 0, 52);
                    ...
                    if (issuance_1.hasTokenAmount(input.issuance)) {
                        ...
                        const issuedTokenCommitment = yield confidential.assetCommitment(token, blindingFactorsToken.assetBlindingFactor);
                        const tokenValueCommitment = yield confidential.valueCommitment(blindingFactorsToken.value, issuedTokenCommitment, blindingFactorsToken.valueBlindingFactor);
                        const inflationRangeProof = yield confidential.rangeProof(blindingFactorsToken.value, issuanceBlindingPrivKeys[inputIndex].tokenKey, token, blindingFactorsToken.assetBlindingFactor, blindingFactorsToken.valueBlindingFactor, tokenValueCommitment, Buffer.alloc(0), '1', 0, 52);
    ...
    blindOutputsRaw(blindingData, blindingPubkeys, outputIndexes, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const outputsBlindingData = yield computeOutputsBlindingData(blindingData, outputsData);
            ...
            for (const outputIndex of outputIndexes) {
                ...
                // commitments
                const assetCommitment = yield confidential.assetCommitment(outputBlindingData.asset, outputBlindingData.assetBlindingFactor);
                const valueCommitment = yield confidential.valueCommitment(outputBlindingData.value, assetCommitment, outputBlindingData.valueBlindingFactor);
                // proofs
                const rangeProof = yield confidential.rangeProofWithNonceHash(outputBlindingData.value, blindingPubkeys[indexInArray], ephemeralPrivKey, outputBlindingData.asset, outputBlindingData.assetBlindingFactor, outputBlindingData.valueBlindingFactor, valueCommitment, this.__CACHE.__TX.outs[outputIndex].script);
                const surjectionProof = yield confidential.surjectionProof(outputBlindingData.asset, outputBlindingData.assetBlindingFactor, blindingData.map(({ asset }) => asset), blindingData.map(({ assetBlindingFactor }) => assetBlindingFactor), randomSeed);
    ...
    rawBlindOutputs(blindingDataLike, blindingPubkeys, issuanceBlindingPrivKeys = [], outputIndexes, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const inputsBlindingData = yield Promise.all(blindingDataLike.map((data, i) => toBlindingData(data, witnesses[i])));
            ...
            yield this.blindOutputsRaw(totalBlindingData, blindingPubkeys, outputIndexes, opts);
...
function computeOutputsBlindingData(inputsBlindingData, outputsData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const finalAmountBlinder = yield confidential.valueBlindingFactor(inputsValues, outputsValues, inputsAssetBlinders, outputsAssetBlinders, inputsAmountBlinders, outputsAmountBlinders);
