...
class Psbt {
    ...
    blindOutputs(blindingDataLike, blindingPubkeys, opts) {
        return this.rawBlindOutputs(blindingDataLike, blindingPubkeys, undefined, opts);
    }
    blindOutputsByIndex(inputsBlindingData, outputsBlindingPubKeys, opts) {
        ...
        return this.rawBlindOutputs(blindingPrivKeysArgs, blindingPublicKey, outputIndexes, opts);
    }
    ...
    async rawBlindOutputs(blindingDataLike, blindingPubkeys, outputIndexes, opts) {
        ...
        const outputsBlindingData = await computeOutputsBlindingData(inputsBlindingData, outputsData);
        ...
        for (const outputIndex of outputIndexes) {
            ...
            // proofs
            const rangeProof = await confidential.rangeProof(outputBlindingData.value, blindingPubkeys[indexInArray], ephemeralPrivKey, outputBlindingData.asset, outputBlindingData.assetBlindingFactor, outputBlindingData.valueBlindingFactor, valueCommitment, c.__TX.outs[outputIndex].script);
            const surjectionProof = await confidential.surjectionProof(outputBlindingData.asset, outputBlindingData.assetBlindingFactor, inputsBlindingData.map(({ asset }) => asset), inputsBlindingData.map(({ assetBlindingFactor }) => assetBlindingFactor), randomSeed);
