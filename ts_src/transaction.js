...
class Transaction {
    ...
    addOutput(scriptPubKey, value, asset, nonce, rangeProof, surjectionProof) {
        typeforce(types.tuple(types.Buffer, types.oneOf(types.ConfidentialValue, types.ConfidentialCommitment, types.BufferOne), types.oneOf(types.ConfidentialCommitment, types.BufferOne), types.oneOf(types.ConfidentialCommitment, types.BufferOne), types.maybe(types.Buffer), types.maybe(types.Buffer)), arguments);
    ...
    hashForWitnessV0(inIndex, prevOutScript, value, hashType) {
        ...
        function writeIssuances(ins) {
            const issuanceSize = ins.reduce((sum, txIn) => !types.Null(txIn.issuance)
                ? sum +
        ...
        function writeOutputs(outs) {
            const outsSize = outs.reduce((sum, txOut) => sum +
                txOut.asset.length +
                txOut.value.length +
                txOut.nonce.length +
                varSliceSize(txOut.script), 0);
